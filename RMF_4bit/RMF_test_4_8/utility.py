        ...
        def bg_target(queue):
            while True:
                if not queue.empty():
                    ...
                    cv2.imwrite(filename, np.array(tensor.numpy(), dtype=np.uint8))
...
def calc_psnr(hbdq, hbd, scale, rgb_range, benchmark=False):
    diff = (hbdq - hbd) / rgb_range
    shave = 0
    mse = diff.pow(2).mean()
